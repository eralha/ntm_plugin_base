...
defined('ABSPATH') or die('Restricted access');
	...
	class er_candidaturas extends er_base_plugin {
		...
		function deleteCandidatura(){
			...
			$query = "SELECT vchFicheiroCv FROM $this->table_candidaturas ";
			...
			if($results){
				unlink($this->uploadDir . $results[0]->vchFicheiroCv);
			}
		...
		function saveCandidatura(){
			...
			if($results != 0){
				...
				if($_FILES['ficheiroCv']){
					...
					if($filePath === 'FILE_SIZE_ERROR' || $filePath === 0){
						wp_die($filePath);
					}
					...
					$msg->vchFicheiroAttachment = $this->uploadDir . $filePath;
				}else{
					//wp_die($id);
				}

				//o envio do mail demora algum tempo, enviamos logo o id
				echo $id;
